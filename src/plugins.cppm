...
// mcpp.plugins.island -- the boundary a device island is reached across.
//
...
// want it removed.
export namespace mcpp::plugins::island {
...
inline std::optional<emitted> emit(std::span<const std::string> entries,
    ...
    if (opt.module_name.empty() || opt.out_dir.empty()) {
        std::cerr << "mcpp.plugins.island: module_name and out_dir are required\n";
        return std::nullopt;
    }
    const auto by  = opt.produced_by.empty() ? std::string("mcpp.plugins.island")
                                             : opt.produced_by;
    const auto dir = std::filesystem::path(opt.out_dir);
    ...
    for (auto const& e : entries) {
        ...
        if (n.empty()) {
            std::cerr << std::format(
                "mcpp.plugins.island: cannot find an entry point name in `{}`.\n"
    ...
    std::string h;
    h += std::format("// Generated by mcpp.plugins.island for {0}. Do not edit.\n"
                     "//\n"
    ...
    if (!surface::write_if_different(out.header_file, h)) {
        std::cerr << std::format("mcpp.plugins.island: cannot write {}\n", out.header_file);
        return std::nullopt;
    }
    if (!opt.emit_module) return out;

    std::string m;
    m += std::format("// Generated by mcpp.plugins.island for {0}. Do not edit.\n"
                     "//\n"
    ...
    if (!surface::write_if_different(out.interface_file, m)) {
        std::cerr << std::format("mcpp.plugins.island: cannot write {}\n", out.interface_file);
        return std::nullopt;
    }
    return out;
